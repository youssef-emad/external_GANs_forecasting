from generators import G_CNN_RNN
from discriminators import D_CNN
...
def parse_args():
    parser = argparse.ArgumentParser(description="Background Mining")
    parser.add_argument(
        "--config-path",
        required=True,
...
if __name__ == "__main__":
    ...
    generator = G_CNN_RNN(
        ...
        noise_size=cfg.generator["noise_window_size"],
    ...
    discriminator = D_CNN(
        total_sequence_length=cfg.experiment["input_window_size"]
        + cfg.experiment["output_window_size"],
        condition_size=cfg.experiment["input_window_size"],
        latent_size=cfg.discriminator["latent_size"],
        n_filters=cfg.discriminator["n_filters"],
        kernel_size=cfg.discriminator["kernel_size"],
        pool_size=cfg.discriminator["pool_size"],
        mean=training_dataset.mean,
        std=training_dataset.std,
    ).to(DEVICE)

    # define conditional GAN
    forGan = CGAN(generator=generator, discriminator=discriminator, device=DEVICE)
